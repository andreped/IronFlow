...
void main() {
  ...
  group('DatabaseHelper Tests', () {
    test('Should initialize database and create tables', () async {
      ...
      when(mockDatabase.execute('CREATE TABLE exercises(id INTEGER PRIMARY KEY AUTOINCREMENT, exercise TEXT, weight TEXT, reps INTEGER, sets INTEGER, timestamp TEXT)'))
          .thenAnswer((_) async => Future.value());

      when(mockDatabase.execute('CREATE TABLE IF NOT EXISTS predefined_exercises(id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT)'))
          .thenAnswer((_) async => Future.value());
    ...
    test('Should check for new high score correctly', () async {
      ...
      // Act
      final isNewHighScore = await databaseHelper.isNewHighScore('Bench Press', 110, 8);
    ...
    test('Should calculate total weight for a day correctly', () async {
      ...
      // Act
      final totalWeight = await databaseHelper.getTotalWeightForDay(DateTime.now());
    ...
    test('Should get the last logged exercise correctly', () async {
      ...
      // Act
      final lastLoggedExercise = await databaseHelper.getLastLoggedExercise('Bench Press');
